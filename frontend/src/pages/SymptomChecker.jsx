...
function SymptomChecker() {
  ...
  const backendUrl = import.meta.env.VITE_FLASK_BACKEND_URL; // from .env
  ...
  const handleCheck = async () => {
    ...
    try {
      const res = await axios.post(`${backendUrl}/check-symptoms`, {
        symptoms: symptoms.split(",").map((s) => s.trim()),
      });

      setResult(res.data); // now backend always returns clean JSON
    } catch (err) {
